...
void TextWidget::rebuild(bool b_rebuild_children)
{
    ...
    {
        if (word.text.empty() && !word.b_newline)
            continue;
...
std::wstring TextWidget::get_word_at_coord(vector2d coord) const
{
    if (coord.x < 0 || coord.y < 0 ||
        coord.x > term_w() || coord.y > term_h())
        return std::wstring();
    ...
    if (y < cells.size())
    {
        const std::vector<tb_cell>& row = cells[y];
        if (x < row.size())
        {
...
int TextWidget::get_index_at_coord(vector2d coord) const
{
    if (coord.x < 0 || coord.y < 0 ||
        coord.x > term_w() || coord.y > term_h())
        return -1;
